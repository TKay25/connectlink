...
def run1(userid):
    ...
    with get_db() as (cursor, connection):
        ...
        datamain['Action'] = datamain['id'].apply(lambda x: f'''<div style="display: flex; gap: 10px;"><a href="/download_contract/{x}" class="btn btn-primary3">Download Contract</a>    <button class="btn btn-primary3 view-project-btn" data-bs-toggle="modal" data-bs-target="#viewprojectModal" data-id="{x}">View Project</button><button class="btn btn-primary3 log-payment-btn" data-bs-toggle="modal" data-bs-target="#logpaymentModal" data-ID="{x}">Notes</button><button class="btn btn-primary3 log-payment-btn" data-bs-toggle="modal" data-bs-target="#logpaymentModal" data-ID="{x}">Log a Payment</button></div>''')
